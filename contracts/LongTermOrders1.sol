...
library LongTermOrdersLib {
        ...
        )
    {
        ...
        //when both pools sell, we use the TWAMM formula
            ...
            int256 k = SafeMathInt.mul(aStart, bStart);
            ...
            int256 endB = SafeMathInt.mul(
                SafeMathInt.div(aStart, endA),
                bStart
            );
    ...
    function computeC(
        ...
        int256 c1 = SafeMathInt.mul(tokenAStart.sqrt(), tokenBIn.sqrt());
        console.log("c1", uint256(c1.toInt()));

        int256 c2 = SafeMathInt.mul(tokenBStart.sqrt(), tokenAIn.sqrt());
        ...
        c = SafeMathInt.div(cNumerator, cDenominator);
    ...
    function computeAmmEndTokenA(
        ...
        //     tokenBIn.sqrt()
        // );
        int256 eNumerator = SafeMathInt.mul(
            SafeMathInt.mul(PRBMathSD59x18.fromInt(4), tokenAIn.sqrt()),
            tokenBIn.sqrt()
        );
        ...
        int256 eDenominator = SafeMathInt.mul(aStart.sqrt(), bStart.sqrt());
        console.log("eDenominator", uint256(eDenominator.toInt()));

        //int256 exponent = eNumerator.mul(eDenominator).exp();
        int256 exponent = SafeMathInt.div(eNumerator, eDenominator).exp();
        ...
        int256 fraction = SafeMathInt.div((exponent + c), (exponent - c));
        ...
        int256 scaling = SafeMathInt
            .div(SafeMathInt.mul(k, tokenAIn), tokenBIn)
            .sqrt();
        console.log("scaling", uint256(scaling.toInt()));

        ammEndTokenA = SafeMathInt.mul(fraction, scaling);
