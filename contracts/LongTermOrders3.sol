...
library LongTermOrdersLib {
        ...
        )
    {
        ...
        //when both pools sell, we use the TWAMM formula
            ...
            int128 aIn = int256(tokenAIn).fromInt();
            int128 bIn = int256(tokenBIn).fromInt();
            int128 aStart = int256(tokenAStart).fromInt();
            int128 bStart = int256(tokenBStart).fromInt();
            int128 k = aStart.mul(bStart);

            int128 c = computeC(aStart, bStart, aIn, bIn);
            int128 endA = computeAmmEndTokenA(aIn, bIn, c, k, aStart, bStart);
            int128 endB = aStart.div(endA).mul(bStart);
            //int256 endB = aStart.mul(bStart).div(endA);
            console.log("endB", endB.toUInt());

            int128 outA = aStart + aIn - endA;
            int128 outB = bStart + bIn - endB;
    ...
    function computeC(
        ...
        int128 tokenAIn,
        int128 tokenBIn
    ) private view returns (int128 c) {
        int128 c1 = tokenAStart.sqrt().mul(tokenBIn.sqrt());
        console.log("c1", c1.toUInt());

        int128 c2 = tokenBStart.sqrt().mul(tokenAIn.sqrt());
        console.log("c2", c2.toUInt());

        int128 cNumerator = c1 - c2;
        console.log("cNumerator", cNumerator.toUInt());

        int128 cDenominator = c1 + c2;
        console.log("cDenominator", cDenominator.toUInt());

        c = cNumerator.div(cDenominator);
        console.log("c", c.toUInt());
    }
    ...
    function computeAmmEndTokenA(
        ...
        int128 aStart,
        int128 bStart
    ) private view returns (int128 ammEndTokenA) {
        //rearranged for numerical stability
        int128 eNumerator = ABDKMath64x64.fromInt(4).mul(tokenAIn).sqrt().mul(
            tokenBIn.sqrt()
        );
        console.log("eNumerator", eNumerator.toUInt());

        int128 eDenominator = aStart.sqrt().mul(bStart.sqrt()).inv();
        console.log("eDenominator", eDenominator.toUInt());

        int128 exponent = eNumerator.mul(eDenominator).exp();
        require(exponent > ABDKMath64x64.abs(c), "Invalid Amount");
        console.log("exponent", exponent.toUInt());

        int128 fraction = (exponent + c).div(exponent - c);
        console.log("fraction", fraction.toUInt());

        int128 scaling = k.div(tokenBIn).sqrt().mul(tokenAIn.sqrt());
        //int256 scaling = k.mul(tokenAIn).sqrt().div(tokenBIn.sqrt());
        console.log("scaling", scaling.toUInt());

        ammEndTokenA = fraction.mul(scaling);
        console.log("ammEndTokenA", ammEndTokenA.toUInt());
    }
}
