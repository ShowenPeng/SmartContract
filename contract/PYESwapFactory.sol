...
contract PYESwapPair is IPYESwapPair, PYESwapERC20 {
    ...
    function initialize(address _token0, address _token1) external {
        ...
        token0 = _token0;
        token1 = _token1;
        routerAddress = IPYESwapRouter(
            IPYESwapFactory(factory).routerAddress()
        );
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "PYESwap: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "PYESwap: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, , ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "PYESwap: INSUFFICIENT_LIQUIDITY"
        );

        if (baseToken != address(0)) {
            require(
                amount0Fee > 0 || amount1Fee > 0,
                "PYESwap: INSUFFICIENT_FEE"
            );
        }
        ...
        require(
            ...
            "PYESwap: INSUFFICIENT_INPUT_AMOUNT"
        );
...
contract PYESwapFactory is IPYESwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(PYESwapPair).creationCode));
